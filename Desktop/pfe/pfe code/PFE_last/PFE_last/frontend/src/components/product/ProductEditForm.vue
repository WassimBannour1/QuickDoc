<template>
  <div class="product-edit-container">
    <div v-if="loading" class="loading-indicator">
      <i class="fas fa-spinner fa-spin"></i> Chargement du produit...
    </div>
    
    <div v-else-if="!product" class="error-state">
      <i class="fas fa-exclamation-circle"></i>
      <p>Produit non trouvé ou erreur lors du chargement</p>
      <router-link to="/admin-entreprise/product-list" class="btn btn-secondary">
          ...
          <div class="form-section">
            ...
            <div class="form-group">
              ...
              <div v-if="selectedCategoryId && filteredSubcategories.length === 0" class="helper-text">
                ...
                <a href="#" @click.prevent="showCategoryManagement">Gérer les sous-catégories</a>
              </div>
            </div>
          </div>
        </div>
...
// Emits
const emit = defineEmits(['product-updated', 'cancel', 'show-category-management']);
...
const filteredSubcategories = computed(() => {
  ...
  const filtered = subcategories.value.filter(subcategory => 
    parseInt(subcategory.category_id) === categoryId
  );
  
  console.log('Filtrage des sous-catégories pour la catégorie:', categoryId);
  console.log('Sous-catégories filtrées:', filtered);
...
const fetchProduct = async () => {
  ...
  try {
    ...
    productForm.value = {
      ...
      subcategory_id: product.value.subcategory_id || ''
    };
    
    // If product has a subcategory, set the selectedCategoryId
    if (product.value.subcategory_id) {
      // Chercher la sous-catégorie correspondante
      const subcategory = subcategories.value.find(s => s.id === parseInt(product.value.subcategory_id));
      if (subcategory) {
        selectedCategoryId.value = subcategory.category_id;
        console.log('Sous-catégorie trouvée:', subcategory);
        console.log('Catégorie parent définie:', selectedCategoryId.value);
      } else {
        console.log('Sous-catégorie non trouvée dans la liste des sous-catégories');
        console.log('ID de sous-catégorie recherché:', product.value.subcategory_id);
        console.log('Sous-catégories disponibles:', subcategories.value);
        
        // Récupérer les informations de la sous-catégorie directement
        fetchSubcategoryDetails(product.value.subcategory_id);
      }
    }
  } catch (error) {
...
const fetchSubcategories = async () => {
  try {
    // Utiliser le bon chemin d'API pour les sous-catégories
...
const fetchSubcategoryDetails = async (subcategoryId) => {
  try {
    // Utiliser le bon chemin d'API pour les sous-catégories
    const response = await api.get(`/api/categories/subcategories/${subcategoryId}`);
    const subcategoryData = response.data.data;
    
    if (subcategoryData) {
      console.log('Détails de la sous-catégorie récupérés:', subcategoryData);
      
      // Ajouter la sous-catégorie à la liste si elle n'existe pas déjà
      if (!subcategories.value.some(s => s.id === subcategoryData.id)) {
        subcategories.value.push(subcategoryData);
      }
      
      // Définir la catégorie parente
      selectedCategoryId.value = subcategoryData.category_id;
      console.log('Catégorie parente définie depuis les détails:', selectedCategoryId.value);
    }
  } catch (error) {
...
const updateProduct = async () => {
  ...
  try {
    ...
    // Update product
    const response = await api.put(`/api/products/${props.id}`, formData);
    
    // Redirect to product list instead of emitting event
    ...
    if (error.response && error.response.data) {
      ...
      if (error.response.status === 409 && error.response.data.field === 'name') {
        ...
        // Show general error
        alert(error.response.data.message || 'Une erreur est survenue lors de la mise à jour du produit');
      }
    } else {
...
// Watch for changes in the product ID
watch(() => props.id, () => {
  if (props.id) {
    fetchProduct();
  }
...
onMounted(async () => {
  try {
    // Charger d'abord les catégories et sous-catégories
  await Promise.all([
    fetchCategories(),
    fetchSubcategories()
  ]);
  
    console.log('Catégories et sous-catégories chargées');
    
    // Puis charger les données du produit
  if (props.id) {
    await fetchProduct();
    }
  } catch (error) {
...
.loading-indicator i, .error-state i {
  font-size: 3rem;
  margin-bottom: 10px;
  color: #bdc3c7;
}
