<template>
  <div class="custom-templates-view">
    <div v-if="loading" class="loading-container">
      <div class="spinner"></div>
      <p>Chargement des modèles personnalisés...</p>
    </div>
...
export default {
  ...
  created() {
    ...
    this.checkTemplateService()
      ...
      .catch(error => {
        ...
        this.error = true;
        this.errorMessage = 'Impossible de charger les modèles personnalisés. Veuillez réessayer plus tard.';
        this.loading = false;
      });
  ...
  methods: {
    async checkTemplateService() {
      try {
        console.log('Checking custom template service availability...');
        const response = await getCustomTemplates();
        console.log('Custom template service response:', response);
        
        // La fonction getCustomTemplates retourne maintenant toujours un objet avec success
        if (!response.success) {
        ...
        if (!response.data || response.data.length === 0) {
          console.log('No custom templates found, but API is working');
        } else {
          console.log(`Found ${response.data.length} custom templates`);
        }
        
        return true;
      } catch (error) {
        console.error('Custom template service check failed:', error);
        throw error;
      }
    },
    async retry() {
    ...
    createNew() {
      this.$router.push('/admin-entreprise/create-template');
    },
    goBack() {
    ...
    handleEditTemplate(template) {
      console.log('CustomTemplatesView: handleEditTemplate called with template:', template);
      // Navigate to the dedicated edit-template route with the template ID
      this.$router.push({
        name: 'edit-template',
